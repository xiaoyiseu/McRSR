...
from models.McRSR import McRSR
...
class ALBEF(nn.Module):
    ...
    def forward(self, image1, image2, text1, text2, alpha, idx, replace):
        ...
        elif self.mode=='McRSR':
            loss_i2t = McRSR(sim_i2t, sim_i2t_targets, idx.squeeze()).mean()
            loss_t2i = McRSR(sim_t2i, sim_t2i_targets, idx.squeeze()).mean() 
            loss_i2i = McRSR(sim_i2i, sim_i2i_targets, idx.squeeze()).mean()
            loss_t2t = McRSR(sim_t2t, sim_t2t_targets, idx.squeeze()).mean()            
            loss_cl  = (loss_i2t + loss_t2i + loss_i2i + loss_t2t) / 4

        elif self.mode=='RASA+McRSR':
            loss_i2t1 = -torch.sum(F.log_softmax(sim_i2t, dim=1) * sim_i2t_targets, dim=1).mean()
            loss_t2i1 = -torch.sum(F.log_softmax(sim_t2i, dim=1) * sim_t2i_targets, dim=1).mean()
            loss_i2i1 = -torch.sum(F.log_softmax(sim_i2i, dim=1) * sim_i2i_targets, dim=1).mean()
            loss_t2t1 = -torch.sum(F.log_softmax(sim_t2t, dim=1) * sim_t2t_targets, dim=1).mean()
            loss_cl1 = (loss_i2t1 + loss_t2i1 + loss_i2i1 + loss_t2t1) / 4

            loss_i2t2 = McRSR(sim_i2t, sim_i2t_targets, idx.squeeze()).mean()
            loss_t2i2 = McRSR(sim_t2i, sim_t2i_targets, idx.squeeze()).mean() 
            # loss_i2i2 = McRSR(sim_i2i, sim_i2i_targets, idx.squeeze()).mean()
            # loss_t2t2 = McRSR(sim_t2t, sim_t2t_targets, idx.squeeze()).mean()            
            loss_cl2  = (loss_i2t2 + loss_t2i2) / 4
        ...
        with torch.no_grad():
            bs = image1.size(0)
            weights_i2t = F.softmax(sim_i2t[:, :bs], dim=1).clamp(min=1e-6)
            weights_t2i = F.softmax(sim_t2i[:, :bs], dim=1).clamp(min=1e-6)
