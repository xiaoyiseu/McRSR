...
class SimpleTokenizer(object):
    def __init__(self, bpe_path: str = default_bpe()):
        ...
        # 检查并确保 '[UNK]' 存在于编码器中
        if '[UNK]' not in self.encoder:
            self.encoder['[UNK]'] = len(self.encoder)  # 手动添加一个未知词汇索引
    ...
    def encode(self, text):
        ...
        for token in re.findall(self.pat, text):
            ...
            bpe_tokens.extend(self.encoder.get(bpe_token, self.encoder['[UNK]']) for bpe_token in self.bpe(token).split(' '))
        return bpe_tokens
