...
class CUHKPEDES(BaseDataset):
    ...
    def _split_anno(self, anno_path: str):
        ...
        return train_annos, test_annos, val_annos#将json文件中的每条的完整信息添加值列表，列表中元素为字典

    #annos: List[dict]-类型注解（type hinting）
    # 表示参数 annos 应该是一个列表，其中每个元素都是字典类型。
    def _process_anno(self, annos: List[dict], training=False):#返回整理顺序后的数据集，数据的索引
        pid_container = set()
        if training:
            dataset = []
            image_id = 0
            for anno in annos:
                ...
                image_id += 1
            # for idx, pid in enumerate(pid_container):#检查
